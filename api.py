...
def detect(request: DetectRequest):
    bundle = load_bundle(MODEL_DIR)
    outputs, risks, explanations = {}, [], []
    for kind, value in (("url", request.url), ("text", request.text), ("html", request.html)):
        if value:
            score, reasons = predict(kind, value, bundle)
            outputs[kind] = {"risk_score": round(score, 4), "label": "suspicious" if score >= .5 else "likely_safe", "explanations": reasons}
            risks.append(score); explanations.extend(f"{kind}: {r}" for r in reasons)
    score = max(risks) if risks else 0.0
    return {"risk_score": round(score, 4), "label": "suspicious" if score >= .5 else "likely_safe",
            "components": outputs, "explanations": explanations[:10],
            "models_available": sorted(bundle), "disclaimer": "Heuristic model output; verify independently."}
